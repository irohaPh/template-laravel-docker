...
# masterをロックする
mysql -u root --password=root -h mysql-master -e "RESET MASTER;"
mysql -u root --password=root -h mysql-master -e "FLUSH TABLES WITH READ LOCK;"

# masterのDB情報をDumpする
# ここでは --all-databases にしてるけど用途に応じて必要なDBだけにしていいと思う
mysqldump -uroot -proot -h mysql-master --all-databases --master-data --single-transaction --flush-logs --events > /tmp/master_dump.sql
...
log_file=`mysql -u root --password=root -h mysql-master -e "SHOW MASTER STATUS\G" | grep File: | awk '{print $2}'`
pos=`mysql -u root --password=root -h mysql-master -e "SHOW MASTER STATUS\G" | grep Position: | awk '{print $2}'`
...
# masterをunlockする
mysql -u root --password=root -h mysql-master -e "UNLOCK TABLES;"